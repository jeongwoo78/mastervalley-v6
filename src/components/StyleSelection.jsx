// PicoArt v51 - StyleSelection (미술사조 11개, 작은 카드 디자인) + 카테고리 스와이프
...
const StyleSelection = ({ onSelect }) => {
  ...
  const touchRef = useRef({ startX: 0, startY: 0, isDragging: false, isHorizontal: null, dragOffset: 0 });
  const categoryOrder = ['movements', 'masters', 'oriental'];
  const currentIndex = categoryOrder.indexOf(mainCategory);
  ...
  const mainCategories = {
    movements: {
      name: '미술사조', icon: '🎨', description: '서양 미술의 흐름',
    ...
    masters: {
      name: '거장 컬렉션', icon: '⭐', description: '시대를 대표하는 거장들',
      subcategories: ['masters']
    },
    oriental: {
      name: '동양화', icon: '🎎', description: '한·중·일 전통 미술',
  ...
  const handleTouchMove = useCallback((e) => {
    ...
    if (t.isHorizontal === null && (Math.abs(dx) > 8 || Math.abs(dy) > 8)) {
    ...
    if (trackRef.current) {
      const idx = categoryOrder.indexOf(mainCategory);
      const vw = trackRef.current.parentElement.offsetWidth;
      let pct = -idx * 100 + (dx / vw) * 100;
      // 경계 저항
      if ((idx === 0 && dx > 0) || (idx === 2 && dx < 0)) {
        pct = -idx * 100 + (dx * 0.3 / vw) * 100;
      }
  ...
  const handleTouchEnd = useCallback(() => {
    ...
    const idx = categoryOrder.indexOf(mainCategory);
    if (t.dragOffset < -60 && idx < 2) {
      handleMainCategoryChange(categoryOrder[idx + 1]);
    } else if (t.dragOffset > 60 && idx > 0) {
      handleMainCategoryChange(categoryOrder[idx - 1]);
    }
    // else: 원래 위치 복원 (currentIndex 기반 transform)
  }, [mainCategory]);
  ...
  const renderMovements = () => (
    <div className="styles-section">
      ...
      <button className="full-transform-btn"
        style={{ background: 'linear-gradient(135deg, #a5d8ff 0%, #74c0fc 100%)', boxShadow: '0 4px 15px rgba(165, 216, 255, 0.4)', color: '#1e293b' }}
        onClick={() => onSelect({ id: 'movements-all', name: '2,500년 서양미술사', category: 'movements', isFullTransform: true, count: 11 })}
      >
      ...
      <div className="sub-category-tabs">
        {mainCategories.movements.subcategories.map(key => {
          const category = styleCategories[key];
          const styleData = artStyles.find(s => s.id === key || s.category === key);
          if (!category) return null;
          return (
            <button key={key} className={`sub-category-tab ${subCategory === key ? 'active' : ''}`}
  ...
  // ─── 거장/동양화 콘텐츠 ───
  const renderStylesGrid = (cat) => {
    const sub = mainCategories[cat].subcategories[0];
    const group = groupedStyles[sub];
    if (!group) return null;

    return (
      <div className="styles-section">
        <div className="section-header">
          <h2>{cat === 'masters' ? '🎨 거장을 선택하세요' : '🎨 동양화를 선택하세요'}</h2>
          <p className="section-price">{cat === 'masters' ? '$0.25/변환' : '$0.20/변환'}</p>
        </div>

        {cat === 'masters' && (
          <button className="full-transform-btn"
            style={{ background: 'linear-gradient(135deg, #ffc9c9 0%, #ffa8a8 100%)', boxShadow: '0 4px 15px rgba(255, 201, 201, 0.4)', color: '#1e293b' }}
            onClick={() => onSelect({ id: 'masters-all', name: '일곱 거장의 세계', category: 'masters', isFullTransform: true, count: 7 })}
          >
            <span className="full-transform-icon">✨</span>
            <div className="full-transform-content">
              <span className="full-transform-title">전체 변환</span>
        ...
        {cat === 'oriental' && (
          <button className="full-transform-btn"
            style={{ background: 'linear-gradient(135deg, #fcc2d7 0%, #f783ac 100%)', boxShadow: '0 4px 15px rgba(252, 194, 215, 0.4)', color: '#1e293b' }}
            onClick={() => onSelect({ id: 'oriental-all', name: '동아시아 천 년의 미학', category: 'oriental', isFullTransform: true, count: 3 })}
          >
            <span className="full-transform-icon">✨</span>
            <div className="full-transform-content">
              <span className="full-transform-title">전체 변환</span>
        ...
        <div className="styles-grid">
          {group.styles.map(style => (
            <button key={style.id} className="style-card" onClick={() => onSelect(style)}>
              ...
              <div className="card-content">
                ...
                {style.artist && (
                  <div className="artist-info">
                    <span className="artist-name">{style.artist.name}</span>
                    {style.artist.lifespan && <span className="artist-lifespan">{style.artist.lifespan}</span>}
                  </div>
                )}
                <p className="card-description">{style.description}</p>
                {style.model && <div className="model-badge">{style.model === 'FLUX' ? '⚡ FLUX' : '🚀 SDXL'}</div>}
              </div>
            </button>
          ))}
        </div>
    ...
    <div className="style-selection">
      <div className="selection-container">
        ...
        >
          ...
          >
            <div className="swipe-page">{renderMovements()}</div>
            <div className="swipe-page">{renderStylesGrid('masters')}</div>
            <div className="swipe-page">{renderStylesGrid('oriental')}</div>
          </div>
        </div>
      ...
      <style>{`
        ...
        .main-category-tab .tab-icon { font-size: 2.5rem; }
        .main-category-tab .tab-name { font-size: 1.3rem; font-weight: 700; }
        .main-category-tab .tab-desc { font-size: 0.9rem; opacity: 0.9; }
        ...
        .sub-category-tab .tab-icon { font-size: 2.5rem; }
        .sub-category-tab .tab-name { font-size: 1.05rem; font-weight: 700; }
        .sub-category-tab.active .tab-name { color: #667eea; }
        .sub-category-tab .tab-period { font-size: 0.8rem; opacity: 0.75; }
        .sub-category-tab.active .tab-period { color: #667eea; opacity: 0.8; }
        ...
        .section-header h2 { font-size: 1.8rem; color: #2d3748; margin: 0 0 0.5rem 0; }
        .section-price { font-size: 1rem; color: #7c3aed; font-weight: 600; margin: 0; }
        ...
        .full-transform-btn:hover { transform: translateY(-3px); }
        .full-transform-icon { font-size: 2rem; }
        .full-transform-content {
          display: flex; flex-direction: column;
          align-items: flex-start; gap: 0.25rem;
        }
        .full-transform-title { font-size: 1.1rem; font-weight: 700; color: #1e293b; }
        .full-transform-desc { font-size: 0.9rem; color: rgba(30, 41, 59, 0.8); text-align: left; }
        ...
        .style-card {
          ...
          align-items: center; gap: 0.5rem; text-align: center; min-height: 120px;
        }
        .style-card:hover {
          border-color: #667eea;
        ...
        .card-icon { font-size: 2rem; text-align: center; }
        .card-content { display: flex; flex-direction: column; gap: 0.25rem; width: 100%; }
        .card-header h3 { font-size: 0.95rem; color: #2d3748; margin: 0; font-weight: 600; }
        .card-english { font-size: 0.7rem; color: #718096; margin: 0; }
        .artist-info { display: flex; flex-direction: column; gap: 0.15rem; font-size: 0.7rem; color: #718096; }
        .artist-name { font-size: 0.7rem; font-weight: 600; color: #4a5568; }
        .artist-lifespan { font-size: 0.65rem; color: #a0aec0; }
        .card-description {
          font-size: 0.7rem; color: #4a5568; line-height: 1.4; margin: 0;
        ...
        @media (max-width: 768px) {
          .style-selection { padding: 1rem; }
          .main-category-tabs { grid-template-columns: 1fr; gap: 0.75rem; }
          .main-category-tab { padding: 1.25rem; }
          .main-category-tab .tab-icon { font-size: 2rem; }
          .main-category-tab .tab-name { font-size: 1.1rem; }
          .sub-category-tabs { grid-template-columns: repeat(2, 1fr); }
          .styles-section { padding: 1.5rem; }
          .section-header h2 { font-size: 1.5rem; }
          .styles-grid { grid-template-columns: repeat(2, 1fr); gap: 0.75rem; }
          .style-card { padding: 0.75rem; min-height: 100px; }
          .card-icon { font-size: 1.5rem; }
          .card-header h3 { font-size: 0.85rem; }
          .card-description { font-size: 0.65rem; }
          .full-transform-btn { padding: 1rem; }
          .full-transform-icon { font-size: 1.5rem; }
          .full-transform-title { font-size: 1rem; }
          .full-transform-desc { font-size: 0.8rem; }
        }
      `}</style>
    </div>
  );
